# -*- coding: utf-8 -*-
"""Seio Frontal e sela vs Dimorfismo - com IC95%

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1EpajdVw4uBBpyXHDPuh3rEhHDp6gFv6g
...
file_path = '/content/tabela_com_NS.xlsx'
...
colunas = ['sex','Height', 'Width',
       'NS=nasium-sella','Area']
...
# --- Mapas de rótulos: original (1/2) <-> binário (0/1) ---
def to_bin(y):   # 1->0, 2->1
    y = np.asarray(y)
    return np.where(y == 2, 1, 0)

def to_orig(yb):  # 0->1, 1->2
    yb = np.asarray(yb)
    return np.where(yb == 1, 2, 1)
...
best_xgb_clf.fit(X_train, y_train_bin)

# --- Predições no TESTE ---
y_pred_test_bin = best_xgb_clf.predict(X_test)
y_pred_test_xgb = to_orig(y_pred_test_bin)  # volta para 1/2
y_prob_test_xgb = best_xgb_clf.predict_proba(X_test)[:, 1]  # prob da classe binária 1 (=> classe 2 original)
...
# >>> Usa y_test_bin aqui para manter 0/1 dentro da função (estrutura mantida) <<<
metrics_test = bootstrap_metrics(best_xgb_clf, X_test, y_test_bin)
...
conf_matrix = confusion_matrix(y_test, y_pred_test_xgb)  # compara nos rótulos originais (1/2)
accuracy_xgb = accuracy_score(y_test, y_pred_test_xgb)
report = classification_report(y_test, y_pred_test_xgb, zero_division=0)

print("Confusion Matrix (Test Data):\n", conf_matrix)
print("Accuracy:", accuracy_xgb)
print("Classification Report (Test Data):\n", report)

kf_xgb = KFold(n_splits=fold, shuffle=True, random_state=RANDOM_STATE)
# cross_val_predict com rótulos binários
y_pred_cv_bin = cross_val_predict(best_xgb_clf, X_train, y_train_bin, cv=kf_xgb)
y_pred_cv_xgb = to_orig(y_pred_cv_bin)  # volta para 1/2
y_prob_cv_xgb = cross_val_predict(best_xgb_clf, X_train, y_train_bin, cv=kf_xgb, method='predict_proba')[:, 1]
...
for train_index, test_index in kf_xgb.split(X_train):
    X_train_fold, X_test_fold = X_train[train_index], X_train[test_index]
    y_train_fold_bin, y_test_fold_bin = y_train_bin[train_index], y_train_bin[test_index]
    best_xgb_clf.fit(X_train_fold, y_train_fold_bin)
    y_pred_fold_bin = best_xgb_clf.predict(X_test_fold)
    # converte para rótulos originais antes de calcular métricas "weighted" por classe
    y_pred_fold = to_orig(y_pred_fold_bin)
    y_test_fold = to_orig(y_test_fold_bin)
    accuracy_cv_scores.append(accuracy_score(y_test_fold, y_pred_fold))
    precision_cv_scores.append(precision_score(y_test_fold, y_pred_fold, average='weighted', zero_division=0))
    recall_cv_scores.append(recall_score(y_test_fold, y_pred_fold, average='weighted'))
    f1_cv_scores.append(f1_score(y_test_fold, y_pred_fold, average='weighted'))
...
conf_matrix_cv = confusion_matrix(y_train, y_pred_cv_xgb)  # rótulos originais
accuracy_cv_xgb = accuracy_score(y_train, y_pred_cv_xgb)
report_cv = classification_report(y_train, y_pred_cv_xgb, zero_division=0)

print("Confusion Matrix (Cross-Validation):\n", conf_matrix_cv)
print("Cross-Validation Accuracy:", accuracy_cv_xgb)
print("Classification Report (Cross-Validation):\n", report_cv)

# --- AUC no TESTE e na CV ---
# Aqui mantemos pos_label=2 (classe positiva original), usando a probabilidade da classe binária 1
fpr_test, tpr_test, _ = roc_curve(y_test, y_prob_test_xgb, pos_label=2)
roc_auc_test_xgb = auc(fpr_test, tpr_test)

fpr_cv, tpr_cv, _ = roc_curve(y_train, y_prob_cv_xgb, pos_label=2)
roc_auc_cv = auc(fpr_cv, tpr_cv)
...
def bootstrap_auc(model, X, y, n_iterations=1000, random_state=RANDOM_STATE):
    ...
    auc_scores = []
    for _ in range(n_iterations):
        X_resampled, y_resampled = resample(X, y)  # y esperado nos rótulos originais 1/2
        y_prob = model.predict_proba(X_resampled)[:, 1]  # prob da classe binária 1 (=> classe 2 original)
        fpr, tpr, _ = roc_curve(y_resampled, y_prob, pos_label=2)
        auc_scores.append(auc(fpr, tpr))
    return auc_scores

auc_scores_test_xgb = bootstrap_auc(best_xgb_clf, X_test, y_test)
auc_ci_test_xgb = np.percentile(auc_scores_test_xgb, [2.5, 97.5])

print(f"Test Data AUC: {roc_auc_test_xgb} (95% CI: {auc_ci_test_xgb})")

def auc_cross_val(model, X, y, cv, pos_label=2):
    auc_scores = []
    for train_index, test_index in cv.split(X):
        # treino com y binário
        y_tr_bin = to_bin(y[train_index])
        model.fit(X[train_index], y_tr_bin)
        y_prob = model.predict_proba(X[test_index])[:, 1]
        fpr, tpr, _ = roc_curve(y[test_index], y_prob, pos_label=pos_label)  # y original 1/2
        auc_scores.append(auc(fpr, tpr))
    return auc_scores

auc_scores_cv_xgb = auc_cross_val(best_xgb_clf, X_train, y_train, kf_xgb)
auc_ci_cv_xgb = np.percentile(auc_scores_cv_xgb, [2.5, 97.5])

print(f"Cross-Validation AUC: {np.mean(auc_scores_cv_xgb)} (95% CI: {auc_ci_cv_xgb})")
...
param_grid = {
    'n_estimators': [100, 300, 600],
    'learning_rate': [0.01, 0.1],
    'max_depth': [-1, 5, 7],          # -1 = sem limite
    'num_leaves': [15, 31, 63],
    'min_child_samples': [10, 20, 40],
    'subsample': [0.8, 1.0],          # (alias: bagging_fraction)
    'colsample_bytree': [0.8, 1.0],   # (alias: feature_fraction)
    'reg_alpha': [0.0, 0.1],
    'reg_lambda': [0.0, 1.0]
    # Se houver desbalanceamento acentuado, você pode incluir:
...
y_pred_test_lgb = best_lgb_clf.predict(X_test)
y_prob_test_lgb = best_lgb_clf.predict_proba(X_test)[:, 1]
...
metrics_test = bootstrap_metrics(best_lgb_clf, X_test, y_test)
...
conf_matrix = confusion_matrix(y_test, y_pred_test_lgb)
accuracy_lgb = accuracy_score(y_test, y_pred_test_lgb)
report = classification_report(y_test, y_pred_test_lgb, zero_division=0)

print("Confusion Matrix (Test Data):\n", conf_matrix)
print("Accuracy:", accuracy_lgb)
print("Classification Report (Test Data):\n", report)

kf_lgb = KFold(n_splits=fold, shuffle=True, random_state=RANDOM_STATE)
y_pred_cv_lgb = cross_val_predict(best_lgb_clf, X_train, y_train, cv=kf_lgb)
y_prob_cv_lgb = cross_val_predict(best_lgb_clf, X_train, y_train, cv=kf_lgb, method='predict_proba')[:, 1]
...
for train_index, test_index in kf_lgb.split(X_train):
    X_train_fold, X_test_fold = X_train[train_index], X_train[test_index]
    y_train_fold, y_test_fold = y_train[train_index], y_train[test_index]
    best_lgb_clf.fit(X_train_fold, y_train_fold)
    y_pred_fold = best_lgb_clf.predict(X_test_fold)
    accuracy_cv_scores.append(accuracy_score(y_test_fold, y_pred_fold))
    precision_cv_scores.append(precision_score(y_test_fold, y_pred_fold, average='weighted', zero_division=0))
    recall_cv_scores.append(recall_score(y_test_fold, y_pred_fold, average='weighted'))
    f1_cv_scores.append(f1_score(y_test_fold, y_pred_fold, average='weighted'))
...
conf_matrix_cv = confusion_matrix(y_train, y_pred_cv_lgb)
accuracy_cv_lgb = accuracy_score(y_train, y_pred_cv_lgb)
report_cv = classification_report(y_train, y_pred_cv_lgb, zero_division=0)

print("Confusion Matrix (Cross-Validation):\n", conf_matrix_cv)
print("Cross-Validation Accuracy:", accuracy_cv_lgb)
print("Classification Report (Cross-Validation):\n", report_cv)

fpr_test, tpr_test, _ = roc_curve(y_test, y_prob_test_lgb, pos_label=2)
roc_auc_test_lgb = auc(fpr_test, tpr_test)
fpr_cv, tpr_cv, _ = roc_curve(y_train, y_prob_cv_lgb, pos_label=2)
roc_auc_cv = auc(fpr_cv, tpr_cv)
...
auc_scores_test_lgb = bootstrap_auc(best_lgb_clf, X_test, y_test)
auc_ci_test_lgb = np.percentile(auc_scores_test_lgb, [2.5, 97.5])

print(f"Test Data AUC: {roc_auc_test_lgb} (95% CI: {auc_ci_test_lgb})")
...
auc_scores_cv_lgb = auc_cross_val(best_lgb_clf, X_train, y_train, kf_lgb)
auc_ci_cv_lgb = np.percentile(auc_scores_cv_lgb, [2.5, 97.5])

print(f"Cross-Validation AUC: {np.mean(auc_scores_cv_lgb)} (95% CI: {auc_ci_cv_lgb})")
...
param_grid = {
    ...
    'depth': [4, 6, 8],
    'l2_leaf_reg': [1.0, 3.0],
    'rsm': [0.8, 1.0],            # amostragem de features por nível (similar ao colsample)
    # Ajustes opcionais:
    # 'bagging_temperature': [0.0, 1.0],   # controle do bootstrap
    # 'class_weights': [{1:1.0, 2:1.0}, {1:1.0, 2:2.0}]  # se houver desbalanceamento
}
...
grid_search = GridSearchCV(estimator=cat_clf, param_grid=param_grid, cv=5, scoring='accuracy')
grid_search.fit(X_train, y_train)
...
y_pred_test_cat = best_cat_clf.predict(X_test)
y_prob_test_cat = best_cat_clf.predict_proba(X_test)[:, 1]
...
metrics_test = bootstrap_metrics(best_cat_clf, X_test, y_test)
...
conf_matrix = confusion_matrix(y_test, y_pred_test_cat)
accuracy_cat = accuracy_score(y_test, y_pred_test_cat)
report = classification_report(y_test, y_pred_test_cat, zero_division=0)

print("Confusion Matrix (Test Data):\n", conf_matrix)
print("Accuracy:", accuracy_cat)
print("Classification Report (Test Data):\n", report)

kf_cat = KFold(n_splits=fold, shuffle=True, random_state=RANDOM_STATE)
y_pred_cv_cat = cross_val_predict(best_cat_clf, X_train, y_train, cv=kf_cat)
y_prob_cv_cat = cross_val_predict(best_cat_clf, X_train, y_train, cv=kf_cat, method='predict_proba')[:, 1]
...
for train_index, test_index in kf_cat.split(X_train):
    X_train_fold, X_test_fold = X_train[train_index], X_train[test_index]
    y_train_fold, y_test_fold = y_train[train_index], y_train[test_index]
    best_cat_clf.fit(X_train_fold, y_train_fold)
    y_pred_fold = best_cat_clf.predict(X_test_fold)
    accuracy_cv_scores.append(accuracy_score(y_test_fold, y_pred_fold))
    precision_cv_scores.append(precision_score(y_test_fold, y_pred_fold, average='weighted', zero_division=0))
    recall_cv_scores.append(recall_score(y_test_fold, y_pred_fold, average='weighted'))
    f1_cv_scores.append(f1_score(y_test_fold, y_pred_fold, average='weighted'))
...
conf_matrix_cv = confusion_matrix(y_train, y_pred_cv_cat)
accuracy_cv_cat = accuracy_score(y_train, y_pred_cv_cat)
report_cv = classification_report(y_train, y_pred_cv_cat, zero_division=0)

print("Confusion Matrix (Cross-Validation):\n", conf_matrix_cv)
print("Cross-Validation Accuracy:", accuracy_cv_cat)
print("Classification Report (Cross-Validation):\n", report_cv)

fpr_test, tpr_test, _ = roc_curve(y_test, y_prob_test_cat, pos_label=2)
roc_auc_test_cat = auc(fpr_test, tpr_test)
fpr_cv, tpr_cv, _ = roc_curve(y_train, y_prob_cv_cat, pos_label=2)
roc_auc_cv = auc(fpr_cv, tpr_cv)
...
auc_scores_test_cat = bootstrap_auc(best_cat_clf, X_test, y_test)
auc_ci_test_cat = np.percentile(auc_scores_test_cat, [2.5, 97.5])

print(f"Test Data AUC: {roc_auc_test_cat} (95% CI: {auc_ci_test_cat})")
...
auc_scores_cv_cat = auc_cross_val(best_cat_clf, X_train, y_train, kf_cat)
auc_ci_cv_cat = np.percentile(auc_scores_cv_cat, [2.5, 97.5])

print(f"Cross-Validation AUC: {np.mean(auc_scores_cv_cat)} (95% CI: {auc_ci_cv_cat})")

fpr_cat, tpr_cat, _ = roc_curve(y_test, y_prob_test_cat, pos_label = 2)
roc_auc_cat = auc(fpr_cat, tpr_cat)

fpr_cv_cat, tpr_cv_cat, _ = roc_curve(y_train, y_prob_cv_cat, pos_label = 2)
roc_auc_cv_cat = auc(fpr_cv_cat, tpr_cv_cat)

fpr_xgb, tpr_xgb, _ = roc_curve(y_test, y_prob_test_xgb, pos_label = 2)
roc_auc_xgb = auc(fpr_xgb, tpr_xgb)

fpr_cv_xgb, tpr_cv_xgb, _ = roc_curve(y_train, y_prob_cv_xgb, pos_label = 2)
roc_auc_cv_xgb = auc(fpr_cv_xgb, tpr_cv_xgb)

fpr_lgb, tpr_lgb, _ = roc_curve(y_test, y_prob_test_lgb, pos_label = 2)
roc_auc_lgb = auc(fpr_xgb, tpr_xgb)

fpr_cv_lgb, tpr_cv_lgb, _ = roc_curve(y_train, y_prob_cv_lgb, pos_label = 2)
roc_auc_cv_lgb = auc(fpr_cv_lgb, tpr_cv_lgb)
...
axs[0].plot(fpr_cat, tpr_cat, color='teal', lw=2, label='CatBoost (AUC = %0.2f, CI 95%%: [%0.2f, %0.2f])' % (roc_auc_cat, auc_ci_test_cat[0], auc_ci_test_cat[1]))
axs[0].plot(fpr_xgb, tpr_xgb, color='darkorange', lw=2, label='XGBoost (AUC = %0.2f, CI 95%%: [%0.2f, %0.2f])' % (roc_auc_xgb, auc_ci_test_xgb[0], auc_ci_test_xgb[1]))
axs[0].plot(fpr_lgb, tpr_lgb, color='olive', lw=2, label='LightGBM (AUC = %0.2f, CI 95%%: [%0.2f, %0.2f])' % (roc_auc_lgb, auc_ci_test_lgb[0], auc_ci_test_lgb[1]))
...
axs[1].plot(fpr_cv_cat, tpr_cv_cat, color='teal', lw=2, label='CatBoost (AUC = %0.2f, CI 95%%: [%0.2f, %0.2f])' % (roc_auc_cv_cat, auc_ci_cv_cat[0], auc_ci_cv_cat[1]))
...
axs[1].plot(fpr_cv_xgb, tpr_cv_xgb, color='darkorange', lw=2, label='XGBoost (AUC = %0.2f, CI 95%%: [%0.2f, %0.2f])' % (roc_auc_cv_xgb, auc_ci_cv_xgb[0], auc_ci_cv_xgb[1]))
...
axs[1].plot(fpr_cv_lgb, tpr_cv_lgb, color='olive', lw=2, label='LightGBM (AUC = %0.2f, CI 95%%: [%0.2f, %0.2f])' % (roc_auc_cv_lgb, auc_ci_cv_lgb[0], auc_ci_cv_lgb[1]))
...
colors = plt.cm.plasma(np.linspace(0.2, 0.9, 8))  # 8 cores para 8 modelos
...
imp_features_ada = best_ada_clf.feature_importances_
imp_features_xgb = best_xgb_clf.feature_importances_
imp_features_lgb = best_lgb_clf.feature_importances_
imp_features_cat = best_cat_clf.get_feature_importance()
...
sorted_indices_xgb = np.argsort(imp_features_xgb)
sorted_indices_lgb = np.argsort(imp_features_lgb)
sorted_indices_cat = np.argsort(imp_features_cat)

# Criando 4x2 plots
fig, axs = plt.subplots(4, 2, figsize=(16, 18))
fig.suptitle('Feature Importance', fontsize=18)

# Lista com tuplas para iterar
model_infos = [
    ('Gradient Boosting', imp_features_gb, sorted_indices_gb),
    ('Logistic Regression', imp_features_lr, sorted_indices_lr),
    ('Decision Tree', imp_features_dt, sorted_indices_dt),
    ('Random Forest', imp_features_rf, sorted_indices_rf),
    ('AdaBoost', imp_features_ada, sorted_indices_ada),
    ('XGBoost', imp_features_xgb, sorted_indices_xgb),
    ('LightGBM', imp_features_lgb, sorted_indices_lgb),
    ('CatBoost', imp_features_cat, sorted_indices_cat)
]

# Loop para plotar cada modelo
for i, (title, importances, sorted_idx) in enumerate(model_infos):
    row, col = divmod(i, 2)
    axs[row, col].barh(features[sorted_idx], importances[sorted_idx], color=colors[i])
    axs[row, col].set_title(title)
    axs[row, col].set_xlabel('Importance')
    axs[row, col].set_ylabel('Feature')
    axs[row, col].grid(axis='x', linestyle='--', alpha=0.7)
    axs[row, col].set_facecolor('#f0f0f0')  # fundo cinza claro
...
# Modelos e probabilidades
model_names = ["gb", "knn", "ada", "svc", "mlp", "lr", "dt", "rf", "xgb", "lgb", "cat"]
...
# Função de comparação bootstrap
...
for model1, model2 in model_pairs:
    ...
    cis.append(ci_diff)
    print(f"Difference in AUC between {model1.upper()} and {model2.upper()}: {mean_diff:.3f} (95% CI: {ci_diff})")
...
colors = ['b' if diff >= 0 else 'r' for diff in mean_diffs]
bar_colors = ['lightblue' if diff >= 0 else 'lightcoral' for diff in mean_diffs]
edge_colors = ['black' if diff >= 0 else 'darkred' for diff in mean_diffs]

# Gráfico de barras com IC 95%
plt.figure(figsize=(22, 12))
bars = plt.bar(model_labels, mean_diffs,
               yerr=[np.abs(lower_bounds - np.array(mean_diffs)),
                     np.abs(upper_bounds - np.array(mean_diffs))],
               capsize=6,
               color=bar_colors,
